<h1 align="center">Bem-Vindo ☕♨





 </h1>

<p align="center">
  Estudante de Análise e Desenvolvimento de Sistemas (UNISINOS) <br>
</p>



</p>

<br>
<a href="www.linkedin.com/in/gabriel-zaluski-carvalho-da-silva-4b9847320" target="_blank">
  <img align="right" src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

### 🥑 Informações
**Residência:**  Atualmente participo de um projeto de Residência em TIC. Eu e meu grupo estamos desenvolvendo um aplicativo e um site para uma cliente em específico. Estamos usando React (Front), Django (Back) e Postgre com Supabas (Banco de Dados).

**Foco:** Meu plano é me especializar em Java. Atualmente aprendo através da universidade e através de cursos e livros por fora.

<br/>


</div>
